...
### A maldição da dimensionalidade — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que $d\to\infty$, mantendo $M=10$ fixo por eixo, o número de células $M^d$ cresce mais rápido do que qualquer conjunto de dados de tamanho polinomial em $d$.
...
### A maldição da dimensionalidade — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✗ Se, em vez de $M$ células por eixo, usássemos apenas $M=2$ (uma grade binária), o problema de crescimento exponencial em $d$ desapareceria.
...
### A maldição da dimensionalidade — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num problema de reconhecimento de imagens em que cada pixel é uma dimensão (por exemplo, $28\times28=784$ dimensões), um histograma multidimensional ingênuo seria ainda mais inviável do que o exemplo de $d=15$ desta aula.
...
### A maldição da dimensionalidade — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como o problema vem do crescimento exponencial de $M^d$, comprar mais capacidade computacional (mais GPUs) resolveria a maldição da dimensionalidade.
...
### Teoria da decisão: risco e regra de Bayes — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que a matriz de perda $L$ se torna a perda 0-1, a regra de Bayes coincide exatamente com a regra do posterior máximo.
...
### Teoria da decisão: risco e regra de Bayes — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✔ Se a matriz de perda $L$ fosse alterada para ter custos muito diferentes entre os tipos de erro, mas a posteriori $p(\mathcal{C}_k\mid\mathbf{x})$ permanecesse a mesma, a regra de decisão ótima poderia mudar, mesmo sem nenhuma mudança na posteriori.
...
### Teoria da decisão: risco e regra de Bayes — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num sistema de aprovação de crédito, se o custo de aprovar um mau pagador for muito maior que o de recusar um bom pagador, a regra ótima recusará crédito mesmo para clientes com posteriori de bom pagador moderadamente alta (ex: $60\%$), não só abaixo de $50\%$.
...
### Teoria da decisão: risco e regra de Bayes — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como o risco de Bayes $R^\star$ é o menor risco esperado dado o modelo verdadeiro, conclui-se que $R^\star=0$ sempre que o modelo $p(\mathbf{x},\mathcal{C}_k)$ usado for corretamente especificado.
...
### Bayes para $K$ classes e modelos generativos — item (a)

**Heurística:** Contrafactual

**Afirmação:** ✗ Se, em vez de $K=2$, tivéssemos $K=5$ classes, a regra "decida pela conjunta maior" deixaria de valer, sendo necessário reformular o argumento de otimalidade da Aula 1 do zero.
...
### Bayes para $K$ classes e modelos generativos — item (b)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que a priori de uma das $K$ classes tende a zero, a probabilidade de o modelo generativo sortear essa classe tende a zero, mesmo que sua condicional $p(\mathbf{x}\mid\mathcal{C}_k)$ seja idêntica às demais.
...
### Bayes para $K$ classes e modelos generativos — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num sistema de diagnóstico com 5 doenças possíveis mutuamente exclusivas mais "saudável" (6 classes), a evidência $p(\mathbf{x})$ ainda seria a soma da conjunta sobre as 6 classes, só com mais termos do que no caso $K=2$.
...
### Bayes para $K$ classes e modelos generativos — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como substituir a família gaussiana por uma rede neural profunda muda a forma funcional usada para $p(\mathbf{x}\mid\mathcal{C}_k)$, isso também muda o conceito de modelo generativo em si.
...
### O modelo generativo em duas etapas — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que todas as $K$ classes têm a mesma condicional $p(\mathbf{x}\mid\mathcal{C}_k)$, o modelo generativo em duas etapas se reduz, na prática, a sortear $\mathbf{x}$ de uma única distribuição comum, independentemente de qual classe foi sorteada.
...
### O modelo generativo em duas etapas — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✗ Se um modelo discriminativo fosse usado no lugar do modelo generativo, ainda seria possível gerar novos dados sintéticos $\mathbf{x}$ amostrando do modelo ajustado, exatamente como no caso generativo.
...
### O modelo generativo em duas etapas — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num sistema que gera imagens sintéticas de rostos realistas, o procedimento é conceitualmente uma amostragem de $p(\mathbf{x})$ (ou $p(\mathbf{x}\mid\mathcal{C}_k)$) — a mesma ideia central do modelo generativo desta aula, com uma família de distribuições muito mais sofisticada.
...
### O modelo generativo em duas etapas — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como um modelo discriminativo modela diretamente a fronteira sem passar por $p(\mathbf{x}\mid\mathcal{C}_k)$, conclui-se que ele modela diretamente $p(\mathbf{x})$, em vez da posteriori.
...
### Generativo, discriminativo e "IA generativa" — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que um classificador discriminativo só precisa prever a classe mais provável (sem uma pontuação de confiança calibrada), ele pode dispensar completamente qualquer estimativa exata de $p(\mathbf{x}\mid\mathcal{C}_k)$ ou de $p(\mathcal{C}_k\mid\mathbf{x})$, bastando aprender a fronteira certa.
...
### Generativo, discriminativo e "IA generativa" — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✔ Se um modelo discriminativo, além de aprender a fronteira, também precisasse necessariamente estimar $p(\mathbf{x})$ como subproduto do treinamento, ele deixaria de ser, por definição, discriminativo.
...
### Generativo, discriminativo e "IA generativa" — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num problema em que se quer não só classificar e-mails como spam, mas também gerar exemplos sintéticos de spam para aumentar o treino, um modelo puramente discriminativo não bastaria para essa segunda tarefa, mesmo sendo excelente na primeira.
...
### Generativo, discriminativo e "IA generativa" — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como um modelo discriminativo "joga fora" a informação de $p(\mathbf{x}\mid\mathcal{C}_k)$ que o generativo usaria, conclui-se que ele sempre precisa de mais parâmetros do que um modelo generativo equivalente para o mesmo problema.
...
### Contraexemplos de independência: causa comum e XOR — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No exemplo de "causa comum" (grátis/ganhador dado a classe), no limite em que a classe deixa de ter qualquer influência sobre as duas palavras, a dependência marginal entre elas desaparece, mesmo mantendo a independência condicional dada a classe.
...
### Contraexemplos de independência: causa comum e XOR — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✗ No exemplo do XOR, se observássemos apenas uma pista parcial sobre $\mathcal{C}$ (por exemplo, "provavelmente $1$", com $70\%$ de confiança), $x_1$ e $x_2$ permaneceriam exatamente tão dependentes quanto no caso de observar $\mathcal{C}=1$ com certeza.
...
### Contraexemplos de independência: causa comum e XOR — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num sistema de recomendação em que a popularidade geral de um produto afeta tanto "número de cliques" quanto "número de compras", essas duas variáveis podem ser marginalmente dependentes mesmo sendo condicionalmente independentes dado o nível de popularidade.
...
### Contraexemplos de independência: causa comum e XOR — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como o XOR mostra que condicionar pode CRIAR dependência entre variáveis originalmente independentes, conclui-se que condicionar em qualquer variável adicional sempre aumenta (ou mantém) a dependência entre duas variáveis, nunca a reduz.
...
### Independência vs. independência condicional — item (a)

**Heurística:** Contrafactual

**Afirmação:** ✗ Se $X_1,X_2$ forem marginalmente independentes mas NÃO condicionalmente independentes dada uma terceira variável $Z$, isso contradiz o exemplo do XOR desta aula, que mostrou exatamente o padrão oposto.
...
### Independência vs. independência condicional — item (b)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que a classe $C$ tem um único valor possível, independência condicional dada $C$ e independência marginal passam a significar exatamente a mesma coisa.
...
### Independência vs. independência condicional — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Em genética, dois genes podem ser fortemente correlacionados na população geral (por estarem ligados a uma etnia comum), mas condicionalmente independentes dentro de qualquer subgrupo étnico específico — a mesma estrutura de "causa comum" do exemplo do e-mail.
...
### Independência vs. independência condicional — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como a independência condicional dada a classe é a suposição central do Naive Bayes, conclui-se que, se essa suposição for violada, o Naive Bayes necessariamente terá desempenho ruim de classificação.
...
### O algoritmo Naive Bayes — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que uma probabilidade condicional estimada é exatamente zero para algum atributo, o *score* logarítmico dessa classe tende a $-\infty$, não importa quão altas sejam as demais probabilidades condicionais.
...
### O algoritmo Naive Bayes — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✗ Se, em vez de suavização, a correção para probabilidade zero fosse simplesmente ignorar aquele atributo no cálculo do *score* daquela instância, isso teria um efeito equivalente à suavização, só calculado de outra forma.
...
### O algoritmo Naive Bayes — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num classificador de sentimento de texto, se a palavra "maravilhoso" nunca apareceu nos exemplos rotulados como "negativo", um Naive Bayes sem suavização atribuiria zero de probabilidade a qualquer review negativo que contenha essa palavra, não importa o resto do texto.
...
### O algoritmo Naive Bayes — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como somar logaritmos evita o problema de multiplicar muitos números pequenos (*underflow* numérico), a soma de logs também resolve, por si só, o problema de uma probabilidade estimada exatamente zero.
...
### A escolha de família por atributo — item (a)

**Heurística:** Contrafactual

**Afirmação:** ✔ Se um atributo categórico com $M=5$ níveis fosse, por engano, modelado como contínuo e Gaussiano, o Naive Bayes ainda produziria uma pontuação numérica para cada classe, mas essa pontuação deixaria de corresponder a uma probabilidade genuína sobre os 5 níveis discretos.
...
### A escolha de família por atributo — item (b)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que um atributo categórico tem $M=2$ níveis, a distribuição Categórica usada para modelá-lo se reduz a uma Bernoulli, com apenas 1 parâmetro livre por classe.
...
### A escolha de família por atributo — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num prontuário médico misto (idade contínua, tipo sanguíneo categórico com 4 níveis, presença de sintoma binária), o Naive Bayes poderia legitimamente usar três famílias de distribuição diferentes para essas três colunas.
...
### A escolha de família por atributo — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como cada atributo contribui seu próprio termo de log-verossimilhança para a soma do passo 3, misturar famílias diferentes por coluna quebra essa soma, pois os termos passam a ter unidades ou escalas incompatíveis.
...
### O caso binário: custo e priori viram um limiar — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que $c_{II}/c_I\to\infty$, o limiar de decisão ótimo tende ao extremo que quase sempre declara a classe associada a evitar o Tipo II, mesmo com posteriori muito baixa a favor dela.
...
### O caso binário: custo e priori viram um limiar — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✔ Se a perda deixasse de ser 0-1 e passasse a ser fortemente assimétrica, a regra de Bayes deixaria de coincidir com a regra do posterior máximo, mesmo que a posteriori $p(\mathcal{C}_k\mid\mathbf{x})$ permanecesse exatamente a mesma.
...
### O caso binário: custo e priori viram um limiar — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num sistema de controle de qualidade industrial, se deixar passar um produto defeituoso custar 20 vezes mais que descartar um produto bom por engano, a regra ótima usará um limiar de posteriori bem abaixo de $50\%$ para declarar "defeituoso".
...
### O caso binário: custo e priori viram um limiar — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como mudar os custos da matriz de perda desloca o limiar de decisão ótimo, conclui-se que mudar os custos também muda a posteriori $p(\mathcal{C}_k\mid\mathbf{x})$ calculada pelo modelo.
...
### Risco de Bayes e a ressalva que importa — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que as densidades condicionais das classes se tornam idênticas, o risco de Bayes $R^\star$ sob perda 0-1 tende ao seu valor máximo possível, $\min(\pi_A,\pi_B)$ — o mesmo teto já visto na Aula 1.
...
### Risco de Bayes e a ressalva que importa — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✗ Se o modelo $p(\mathbf{x},\mathcal{C}_k)$ usado estiver errado (mal especificado), a regra de Bayes calculada a partir desse modelo errado ainda seria, por definição, ótima em relação ao mundo real, não só em relação ao modelo assumido.
...
### Risco de Bayes e a ressalva que importa — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num sistema de aprovação de empréstimos com suposições simplificadas (independência entre variáveis correlacionadas na realidade), a regra de decisão pode classificar bem a maioria dos casos sob perda 0-1, mesmo que as probabilidades de inadimplência estimadas estejam sistematicamente erradas.
...
### Risco de Bayes e a ressalva que importa — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como a regra de Bayes minimiza o risco posterior calculado a partir de $p(\mathcal{C}_k\mid\mathbf{x})$, a teoria da decisão garante, como parte do seu resultado, que esse $p(\mathcal{C}_k\mid\mathbf{x})$ usado é o verdadeiro processo gerador dos dados.
...
### O exemplo do e-mail: "grátis" e "ganhador" — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que $p(\text{grátis}=1\mid\text{spam})$ e $p(\text{grátis}=1\mid\text{ham})$ se tornam iguais, a conjunta marginal $p(\text{grátis}=1,\text{ganhador}=1)$ passa a ser exatamente igual ao produto das marginais, mesmo mantendo a independência condicional dada a classe.
...
### O exemplo do e-mail: "grátis" e "ganhador" — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✔ Se a prevalência de spam fosse $\pi_{\text{spam}}=0{,}99$ em vez de $0{,}5$ (mantendo as mesmas condicionais), a dependência marginal entre "grátis" e "ganhador" seria menor do que no cenário original da aula ($\pi_{\text{spam}}=0{,}5$).
...
### O exemplo do e-mail: "grátis" e "ganhador" — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ O mesmo padrão de "causa comum" explicaria por que, num hospital, "tosse" e "febre" podem ser marginalmente dependentes mesmo sendo condicionalmente independentes dado o diagnóstico (gripe ou não).
...
### O exemplo do e-mail: "grátis" e "ganhador" — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como a independência condicional dada a classe É satisfeita exatamente no exemplo de "grátis"/"ganhador" (foi construído assim), conclui-se que o Naive Bayes sempre estimará bem a posteriori em qualquer conjunto de dados de spam, não só neste exemplo controlado.
