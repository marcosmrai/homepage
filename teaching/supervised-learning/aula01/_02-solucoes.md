...
### Distribuições, priori e fronteira de decisão — item (a)

**Heurística:** Contrafactual

**Afirmação:** ✔ Se as duas classes fossem igualmente frequentes ($\pi_A=\pi_B=0{,}5$) em vez de $\pi_A=0{,}95$, o cruzamento das conjuntas passaria a coincidir exatamente com o cruzamento das condicionais $T_{COND}$.
...
### Distribuições, priori e fronteira de decisão — item (b)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que $\pi_A \to 1$, o cruzamento das conjuntas $T_{CONJ}$ tende a se afastar cada vez mais do cruzamento das condicionais $T_{COND}$, na direção do território de B.
...
### Distribuições, priori e fronteira de decisão — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Considere uma triagem de segurança em aeroportos em que $99\%$ dos passageiros não representam ameaça. Mesmo com condicionais simétricas e bem separadas, cortar sempre no cruzamento das CONDICIONAIS (ignorando essa priori $99\%/1\%$) não é a regra de decisão ótima sob perda 0-1.
...
### Distribuições, priori e fronteira de decisão — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como o cruzamento das condicionais não depende das prioris, conclui-se que a taxa de erro total (Tipo I + Tipo II, ponderados pelas prioris) também não depende de onde as prioris colocam o peso relativo entre os dois tipos de erro.
...
### Teorema de Bayes e o exemplo da triagem médica — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que a prevalência da doença tende a zero, mantendo sensibilidade e falso positivo fixos em $0{,}90$ e $0{,}03$, a posteriori $p(D\mid+)$ também tende a zero.
...
### Teorema de Bayes e o exemplo da triagem médica — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✔ Se a prevalência da doença fosse $50\%$ em vez de $1\%$ (mantendo sensibilidade $0{,}90$ e falso positivo $0{,}03$), a posteriori $p(D\mid+)$ passaria a ser maior que $90\%$.
...
### Teorema de Bayes e o exemplo da triagem médica — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num teste de triagem usado em campanhas de vacinação em massa, com prevalência ainda menor (1 em 10.000) e sensibilidade e especificidade excelentes (99% cada), a proporção de positivos que são realmente doentes seria ainda menor do que no exemplo da aula.
...
### Teorema de Bayes e o exemplo da triagem médica — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como a evidência $p(+)$ é definida como $\sum_k p(+\mid\mathcal{C}_k)p(\mathcal{C}_k)$, ela representa a probabilidade de um resultado positivo vindo especificamente da classe doente.
...
### A distribuição Beta e detecção de anomalias — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que $a\to\infty$ e $b\to\infty$ mantendo $a/(a+b)$ constante, a distribuição Beta se concentra cada vez mais estreitamente em torno de sua média, tendendo a uma distribuição degenerada.
...
### A distribuição Beta e detecção de anomalias — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✔ Se os dados de uma folha realmente seguissem uma Beta em forma de U ($a=0{,}5$, $b=0{,}5$), um estimador de momentos ainda recuperaria corretamente esses parâmetros a partir da média e variância amostrais, mesmo sem assumir nada sobre a forma da distribuição.
...
### A distribuição Beta e detecção de anomalias — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num sistema de recomendação que modela a taxa de cliques (CTR) de cada anúncio, a Beta seria uma escolha tão razoável quanto no exemplo de escores de anomalia desta aula, e sofreria da mesma armadilha se algum anúncio tivesse CTR observada de exatamente $0\%$ ou $100\%$.
...
### A distribuição Beta e detecção de anomalias — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como treinar apenas com dados "normais" permite estimar bem a densidade $p(x\mid\mathcal{C}_A)$, conclui-se que a taxa de detecção (Tipo II) também pode ser calculada diretamente dos dados de treino, sem qualquer suposição adicional sobre as anomalias.
...
### Tipos de erro e o limiar de decisão — item (a)

**Heurística:** Contrafactual

**Afirmação:** ✔ Se, em vez de duas classes unimodais com uma única região de sobreposição, a classe B fosse bimodal (uma "bolha" de densidade de cada lado de A), deslocar o limiar $t$ para a direita poderia, em algum trecho, reduzir simultaneamente as duas taxas de erro.
...
### Tipos de erro e o limiar de decisão — item (b)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que o limiar captura toda a reta como pertencente a uma única classe, a taxa de erro associada a essa classe vai a zero e a do outro tipo vai a $100\%$, independentemente da forma das densidades.
...
### Tipos de erro e o limiar de decisão — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num sistema de triagem de spam, se "Tipo I" for classificar um e-mail legítimo como spam e "Tipo II" for deixar passar um spam verdadeiro, qual rótulo é "Tipo I" depende de qual classe é declarada a hipótese nula — não é uma propriedade fixa do problema.
...
### Tipos de erro e o limiar de decisão — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como aumentar o limiar $t$ tipicamente reduz a taxa de erro Tipo I e aumenta a Tipo II, conclui-se que a soma Tipo I + Tipo II é sempre constante, não importa onde $t$ esteja.
...
### Priori, verossimilhança, evidência e posteriori — item (a)

**Heurística:** Transferência

**Afirmação:** ✔ Se, em vez de duas classes, tivéssemos $K=10$ classes, a evidência $p(x)$ ainda seria calculada somando a conjunta $p(x,\mathcal{C}_j)$ sobre todas as classes, agora com 10 termos.
...
### Priori, verossimilhança, evidência e posteriori — item (b)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que $\pi_A\to 1$, a posteriori $p(\mathcal{C}_A\mid x)$ tende a $1$ para qualquer valor observado de $x$, mesmo que $x$ seja um valor típico da classe B.
...
### Priori, verossimilhança, evidência e posteriori — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num tribunal, se o "prior" for a crença de culpa antes de ver evidência e a "verossimilhança" for o quão bem a evidência se encaixa com a culpa, um prior muito baixo de culpa (presunção de inocência) exige evidência proporcionalmente mais forte para produzir uma posteriori de culpa convincente.
...
### Priori, verossimilhança, evidência e posteriori — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como a posteriori é proporcional ao produto entre verossimilhança e priori, duas classes com a mesma verossimilhança $p(x\mid\mathcal{C}_k)$ num ponto $x$ sempre têm posteriores iguais nesse ponto, independentemente das prioris.
...
### Densidade vs. probabilidade — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que um intervalo $[a,b]$ encolhe para um único ponto, a probabilidade $P(a\le X\le b)$ tende a zero, mesmo que a densidade $p(a)$ seja um número grande (por exemplo, $1000$).
...
### Densidade vs. probabilidade — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✔ Se $X$ fosse uma variável aleatória discreta em vez de contínua, a afirmação "$P(X=x)=0$ para qualquer ponto isolado" deixaria de valer em geral.
...
### Densidade vs. probabilidade — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Numa distribuição Uniforme$[0,\,10^{-6}]$, a densidade dentro do intervalo vale $10^6$, e isso não viola nenhum axioma de probabilidade, pois a integral sobre todo o domínio continua valendo exatamente $1$.
...
### Densidade vs. probabilidade — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como a densidade $p(x)$ pode ultrapassar $1$ sem problema algum, conclui-se que ela também pode ser negativa em algum ponto, desde que a integral total sobre o domínio ainda valha $1$.
...
### Mudança de variável e o Jacobiano — item (a)

**Heurística:** Contrafactual

**Afirmação:** ✗ Se a transformação $y=g(x)$ não fosse bijetora (por exemplo, $y=x^2$ com $x$ variando em toda a reta real), a fórmula $p_Y(y)=p_X(g^{-1}(y))|dx/dy|$ ainda poderia ser aplicada diretamente, sem nenhuma modificação.
...
### Mudança de variável e o Jacobiano — item (b)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que a derivada $g'(x)\to 0$ num ponto, a densidade transformada $p_Y(y)$ nesse ponto tende a $+\infty$ (supondo $p_X$ positiva e finita ali).
...
### Mudança de variável e o Jacobiano — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Ao converter uma distribuição de renda de reais para log(reais), a moda da distribuição pode mudar de posição relativa, mesmo sendo a mesma variável aleatória subjacente, só medida numa escala diferente.
...
### Mudança de variável e o Jacobiano — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como a moda de uma densidade pode mudar sob reparametrização, a decisão de classificação $\arg\max_k p(\mathcal{C}_k\mid x)$ também muda de classe vencedora dependendo de como $x$ é parametrizado.
...
### Momentos de uma distribuição — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que uma distribuição se torna cada vez mais concentrada (variância $\to 0$), sua curtose em excesso não necessariamente tende a zero.
...
### Momentos de uma distribuição — item (b)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como distribuições simétricas em torno da média têm *skewness* zero, conclui-se que *skewness* zero implica necessariamente que a distribuição é simétrica.
...
### Momentos de uma distribuição — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Numa distribuição de renda familiar (cauda longa à direita), espera-se *skewness* positiva, e a média tende a ficar acima da mediana, não abaixo.
...
### Momentos de uma distribuição — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como curtose em excesso zero indica uma cauda comparável à Gaussiana, conclui-se que toda distribuição com curtose em excesso zero deve ser, ela mesma, uma Gaussiana.
...
### Caudas leves vs. pesadas — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✔ Se uma distribuição de cauda pesada fosse usada para modelar erros de medição de um instrumento, eventos a $5\sigma$ de distância da média deixariam de ser "praticamente impossíveis".
...
### Caudas leves vs. pesadas — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Em mercados financeiros, retornos diários de ações costumam ter caudas mais pesadas que a Gaussiana, consistente com o uso de distribuições como a $t$ de Student em vez da Gaussiana para modelar retornos.
...
### Caudas leves vs. pesadas — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como a distribuição $t$ de Student com poucos graus de liberdade tem cauda mais pesada que a Gaussiana, ela necessariamente não tem variância finita, para qualquer valor de $\nu$.
...
### Forma fechada vs. máxima verossimilhança (Beta) — item (a)

**Heurística:** Contrafactual

**Afirmação:** ✗ Se, em vez da Beta, os dados de uma folha realmente seguissem uma distribuição Gaussiana, o MLE dos parâmetros também exigiria um procedimento numérico iterativo, assim como ocorre com a Beta.
...
### Forma fechada vs. máxima verossimilhança (Beta) — item (b)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que $a=b$ (Beta simétrica em torno de $0{,}5$), o estimador de momentos produziria uma estimativa de $a$ igual à de $b$ só se a média amostral for exatamente $0{,}5$.
...
### Forma fechada vs. máxima verossimilhança (Beta) — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Ao ajustar uma Beta por máxima verossimilhança num pacote estatístico, o software provavelmente usa um método numérico iterativo para resolver as equações de verossimilhança, em vez de uma fórmula fechada.
...
### Forma fechada vs. máxima verossimilhança (Beta) — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como o MLE é assintoticamente eficiente e o estimador de momentos, em geral, não é, conclui-se que o estimador de momentos é sempre uma escolha pior, para qualquer tamanho de amostra $N$.
...
### Armadilha de zeros e uns exatos — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que $a\to 0^+$ (mantendo $b$ fixo), a densidade da Beta próxima de $x=0$ diverge cada vez mais rapidamente, tornando uma observação exatamente igual a $0$ ainda mais problemática.
...
### Armadilha de zeros e uns exatos — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✔ Se todos os dados de treino, por coincidência, estivessem estritamente dentro do intervalo aberto $(0,1)$ — sem nenhum zero ou um exato —, a armadilha discutida nesta aula deixaria de ser uma preocupação para aquele conjunto específico, mesmo que $a<1$ ou $b<1$.
...
### Armadilha de zeros e uns exatos — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num conjunto de avaliações de produtos normalizadas para $[0,1]$ (nota de 0 a 5 estrelas dividida por 5), é comum observar exatamente os valores extremos $0$ e $1$, tornando essa armadilha uma preocupação prática real, não só teórica.
...
### Armadilha de zeros e uns exatos — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como o *clipping* para $[\varepsilon,1-\varepsilon]$ é uma correção honesta desde que $\varepsilon$ seja declarado, conclui-se que qualquer escolha de $\varepsilon$ produz resultados de ajuste igualmente bons.
...
### Erro de Bayes e detecção com uma única densidade — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que $p(x\mid\mathcal{C}_A)$ e $p(x\mid\mathcal{C}_B)$ se tornam idênticas em todo o domínio, o erro de Bayes tende ao valor máximo possível para um problema de duas classes.
...
### Erro de Bayes e detecção com uma única densidade — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✗ Um classificador arbitrariamente complexo (bilhões de parâmetros), treinado com dados infinitos, poderia reduzir o erro de Bayes a zero, mesmo havendo sobreposição real entre as densidades das classes.
...
### Erro de Bayes e detecção com uma única densidade — item (c)
...
### Erro de Bayes e detecção com uma única densidade — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como todo número de "taxa de detecção" de um detector treinado só com dados normais depende de alguma suposição sobre as anomalias, conclui-se que esses detectores nunca são úteis na prática, já que a suposição pode estar errada.
