...
### O Viés do Erro de Treino — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que a profundidade da árvore não é limitada e há um ponto de treino distinto por folha, o erro de treino tende a $0\%$, independentemente de quão complexa seja a fronteira real entre as classes.
...
### O Viés do Erro de Treino — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✗ Se a árvore de profundidade 15 tem a mesma acurácia de teste que a árvore de profundidade 6, isso prova que a árvore mais profunda é necessariamente pior no teste do que uma menos profunda.
...
### O Viés do Erro de Treino — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num modelo de previsão de preços de imóveis ajustado com um polinômio de grau muito alto (que passa exatamente por todos os pontos de treino), o erro de treino seria próximo de zero, mas isso não garante nada sobre o erro em imóveis não vistos.
...
### O Viés do Erro de Treino — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como o erro de treino tende a subestimar o erro de generalização, conclui-se que um modelo com erro de treino alto necessariamente generaliza melhor do que um com erro de treino baixo.
...
### Train/Validation/Test — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que o conjunto de teste é consultado repetidas vezes durante o ajuste de hiperparâmetros, a estimativa final de desempenho no teste se torna tão otimista quanto medir no próprio conjunto de treino.
...
### Train/Validation/Test — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✗ Se o conjunto de validação, em vez do de teste, fosse consultado repetidamente para escolher hiperparâmetros, isso não teria custo estatístico algum, pois validação existe exatamente para ser consultada quantas vezes for preciso.
...
### Train/Validation/Test — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Numa competição de ciência de dados, o "*leaderboard* público" funciona como um conjunto de validação consultável repetidamente; competidores que otimizam demais para ele costumam piorar no *leaderboard* privado (o teste verdadeiro) — o *overfitting ao leaderboard*.
...
### Train/Validation/Test — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como o conjunto de treino é usado para ajustar os parâmetros do modelo, conclui-se que o conjunto de validação é usado para ajustar os dados do modelo (limpá-los ou transformá-los), não seus hiperparâmetros.
...
### O Procedimento k-fold — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que $k=N$ (LOOCV), cada fold de validação contém exatamente um único ponto.

**Resposta:** Verdadeiro

**Justificativa:** É a própria definição de LOOCV: com $k=N$ folds, cada fold necessariamente contém $N/k=1$ ponto.

### O Procedimento k-fold — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✔ Se, em vez de treinar $k$ modelos diferentes, k-fold CV usasse um único modelo treinado com todos os dados para avaliar cada fold de validação, a estimativa resultante deixaria de simular uma amostra de teste verdadeiramente independente do ajuste.
...
### O Procedimento k-fold — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num estudo médico com pacientes de 10 hospitais diferentes, se cada fold de uma 5-fold CV misturar aleatoriamente pacientes de todos os hospitais, a CV pode superestimar o desempenho num hospital totalmente novo.
...
### O Procedimento k-fold — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como cada fold de treino em k-fold usa quase os mesmos dados que o treino completo, o modelo ajustado em cada fold é praticamente idêntico ao ajustado com a amostra inteira, então CV mede essencialmente a mesma coisa que o erro de treino.
...
### Estratificação — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que uma das classes representa menos de $1\%$ da amostra, a ausência de estratificação aumenta substancialmente o risco de algum fold não conter nenhum exemplo dessa classe rara.
...
### Estratificação — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✗ Se as classes já estivessem perfeitamente balanceadas ($50\%/50\%$) na amostra completa, usar `StratifiedKFold` em vez de `KFold` comum produziria sempre exatamente os mesmos folds, sem nenhuma diferença possível.
...
### Estratificação — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num problema de imagens médicas com 3 classes muito desbalanceadas ($90\%/9\%/1\%$), a estratificação se torna proporcionalmente mais importante do que num problema balanceado de 2 classes.
...
### Estratificação — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como a estratificação muda quais pontos específicos caem em cada fold, ela também altera o valor esperado da estimativa de CV, tornando-a enviesada.
...
### LOOCV vs. k-fold Pequeno — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que $k\to N$ (o maior valor possível de $k$), cada modelo de LOOCV é treinado com $N-1$ pontos — o máximo de dados de treino possível dentro do esquema de k-fold.
...
### LOOCV vs. k-fold Pequeno — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✗ Se repetíssemos o cálculo de LOOCV várias vezes no mesmo conjunto de dados, obteríamos valores diferentes a cada repetição, assim como acontece com 5-fold e 10-fold quando a partição é sorteada de novo.
...
### LOOCV vs. k-fold Pequeno — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num conjunto de apenas 20 pacientes, LOOCV se torna proporcionalmente mais atraente do que 10-fold, porque deixar de fora 2 pacientes (10%) já é uma perda relativamente maior de dados de treino do que deixar de fora 1.
...
### LOOCV vs. k-fold Pequeno — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como o ESL descreve o estimador de LOOCV como aproximadamente não-enviesado, conclui-se que LOOCV é sempre a melhor estimativa possível do erro de generalização, superando 5-fold e 10-fold em qualidade.
...
### Custo Computacional e Variância de k — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que $N$ é muito grande (milhões de pontos), o custo de LOOCV ($N$ ajustes completos) se torna proibitivo mesmo com ajustes individuais rápidos, tornando $k=5$ ou $10$ escolhas praticamente necessárias.
...
### Custo Computacional e Variância de k — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✔ Se os $N$ conjuntos de treino do LOOCV fossem, ao contrário do que o ESL descreve, bem diferentes entre si, a alta variância do estimador de LOOCV deixaria de ser explicada por esse mecanismo.
...
### Custo Computacional e Variância de k — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Se um outro pesquisador repetisse o experimento desta aula com uma semente aleatória diferente para a partição de 10-fold, a média obtida provavelmente seria ligeiramente diferente, mas dentro da ordem de grandeza do desvio-padrão entre repetições já medido.
...
### Custo Computacional e Variância de k — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como $k=5$ e $k=10$ são mais baratos computacionalmente que LOOCV, conclui-se que eles também produzem, necessariamente, uma estimativa de erro menos precisa do que LOOCV.
...
### Aplicação: Poda por Validação Cruzada — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que $\lambda=0$ (sem nenhuma penalidade), a árvore escolhida seria a árvore completa, sem poda — o extremo oposto da árvore escolhida por CV nesta aula.
...
### Aplicação: Poda por Validação Cruzada — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✔ Se a árvore escolhida por CV (4 folhas) tivesse tido uma acurácia de teste muito pior do que a árvore completa (19 folhas), em vez do empate observado nesta aula, isso enfraqueceria o argumento de preferir a árvore mais simples.
...
### Aplicação: Poda por Validação Cruzada — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num sistema de aprovação de crédito auditado por reguladores, uma árvore de 4 folhas escolhida por CV seria mais fácil de justificar do que uma de 19 folhas, mesmo com a mesma acurácia.
...
### Aplicação: Poda por Validação Cruzada — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como `cost_complexity_pruning_path` gera os candidatos de $\lambda$ de forma determinística a partir do treino, escolher $\lambda$ por inspeção visual de um único gráfico treino/validação já garante a mesma reprodutibilidade que escolher por CV com múltiplos folds.
...
### A Regra de 1 Desvio-Padrão — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que o desvio-padrão entre folds de CV tende a zero, a regra de 1-SE tende a escolher praticamente o mesmo modelo que a regra de máxima média de CV.
...
### A Regra de 1 Desvio-Padrão — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✗ Se a árvore escolhida pela regra de 1-SE tivesse, por coincidência, exatamente a mesma acurácia de teste que a árvore de máxima média de CV, isso provaria que a regra de 1-SE está correta em geral, não só neste experimento.
...
### A Regra de 1 Desvio-Padrão — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Num cenário em que o custo de deploy de um modelo complexo é alto (por exemplo, um dispositivo com pouca memória), a regra de 1-SE seria mais atraente do que neste experimento didático, mesmo custando um pouco de acurácia.
...
### A Regra de 1 Desvio-Padrão — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como a regra de 1-SE reconhece que a estimativa de CV tem incerteza amostral, conclui-se que ela elimina completamente essa incerteza ao escolher o modelo mais simples dentro da faixa.
...
**Afirmação:** ✔ No limite em que o número de réplicas $B\to\infty$, a distribuição empírica das réplicas se aproxima de uma aproximação estável da verdadeira distribuição amostral, mas o tamanho de CADA réplica continua sendo $N$, não crescendo com $B$.
...
**Afirmação:** ✔ Se o Bootstrap reamostrasse SEM reposição, cada réplica gerada seria idêntica à amostra original (mesmos pontos), tornando a técnica inútil para medir variabilidade.
...
**Afirmação:** ✔ Num estudo que mede a mediana (não a média) do tempo de resposta de um sistema, o Bootstrap ainda poderia estimar um intervalo de confiança para essa mediana, mesmo sem fórmula analítica fechada para seu erro-padrão.
...
**Afirmação:** ✗ Como o Bootstrap e a validação cruzada usam ambos técnicas de reamostragem sobre os mesmos dados, conclui-se que respondem exatamente à mesma pergunta estatística.
...
### Duas Aplicações do Bootstrap — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que o conjunto de teste tivesse um número enorme de pacientes (não só 171), o intervalo de confiança da acurácia tenderia a ficar mais estreito do que o observado nesta aula.
...
### Duas Aplicações do Bootstrap — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✗ Se, em vez de reajustar a árvore em cada réplica bootstrap do treino, apenas reavaliássemos a MESMA árvore original em cada réplica, essa variante mediria a mesma coisa que "reamostrar o treino e reajustar".
...
### Duas Aplicações do Bootstrap — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Numa empresa que quer saber tanto a taxa de erro real do seu modelo de fraude implantado quanto o quanto seu processo de treino é sensível à amostra histórica usada, essas duas perguntas exigiriam as duas aplicações distintas do Bootstrap desta aula.
...
### Duas Aplicações do Bootstrap — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como as duas aplicações do Bootstrap desta aula usam a mesma técnica de reamostragem com reposição, elas produzem, necessariamente, o mesmo intervalo de confiança.
...
### Vazamento de Dados (Data Leakage) — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que o número de atributos irrelevantes $p$ cresce (mantendo $N=50$ fixo), o vazamento por seleção antes do split se torna ainda mais grave, pois há mais chances de achar, por acaso, atributos aparentemente correlacionados com o rótulo.
...
### Vazamento de Dados (Data Leakage) — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✔ Se a seleção dos 100 atributos mais correlacionados fosse feita usando só os dados de treino de cada fold, o erro estimado por CV se aproximaria do valor real de $50\%$, em vez do ${\sim}1{,}5\%$ observado no jeito errado.
...
### Vazamento de Dados (Data Leakage) — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Um pipeline que aplica um redutor de dimensionalidade (como PCA) ajustado com a base inteira, antes de particionar em folds, sofre do mesmo tipo de vazamento do experimento de seleção de atributos desta aula, mesmo sem nenhuma seleção explícita.
...
### Vazamento de Dados (Data Leakage) — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como o vazamento de dados infla artificialmente a estimativa de CV para cima, conclui-se que vazamento de dados sempre faz a estimativa de CV parecer PIOR do que o desempenho real do modelo.
...
### Reportando Resultados de Validação Cruzada — item (a)

**Heurística:** Limite

**Afirmação:** ✔ No limite em que o desvio-padrão entre folds de CV tende a zero, reportar só a média já comunica virtualmente toda a informação relevante sobre a incerteza da estimativa.
...
### Reportando Resultados de Validação Cruzada — item (b)

**Heurística:** Contrafactual

**Afirmação:** ✔ Se o ESL não tivesse feito nenhuma recomendação explícita sobre reportar o erro-padrão, ainda assim seria estatisticamente correto reportá-lo, pois a variabilidade entre folds é uma propriedade real da estimativa, independente de qualquer recomendação de livro-texto.
...
### Reportando Resultados de Validação Cruzada — item (c)

**Heurística:** Transferência

**Afirmação:** ✔ Ao comparar dois modelos num artigo científico, reportar "modelo A: 85% de acurácia, modelo B: 84%" sem nenhuma medida de variabilidade não permite ao leitor avaliar se essa diferença é estatisticamente significativa ou só ruído de amostragem.
...
### Reportando Resultados de Validação Cruzada — item (d)

**Heurística:** Falsa equivalência

**Afirmação:** ✗ Como um desvio-padrão alto entre os folds de CV significa que a métrica varia bastante de fold para fold, conclui-se que um desvio-padrão alto indica uma estimativa mais estável e confiável do desempenho do modelo.
